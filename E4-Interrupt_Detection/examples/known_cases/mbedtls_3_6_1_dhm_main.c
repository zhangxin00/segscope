...
static int run_core(int randomize, int iter_index) {
  int rc = 1;
  mbedtls_mpi g;
  mbedtls_mpi x;
  mbedtls_mpi p;
...
static int run_api(int randomize, int iter_index) {
  ...
  if (mbedtls_mpi_read_string(&p, 10, "7919") != 0 ||
      mbedtls_mpi_read_string(&g, 10, "2") != 0) {
    goto cleanup;
  }

  if (mbedtls_dhm_set_group(&alice, &p, &g) != 0 ||
      mbedtls_dhm_set_group(&bob, &p, &g) != 0) {
    goto cleanup;
  }
  ...
  size_t pub_len = mbedtls_dhm_get_len(&alice);
  if (pub_len == 0 || pub_len > sizeof(alice_pub)) {
    goto cleanup;
  }

  if (mbedtls_dhm_make_public(&alice, (int) pub_len, alice_pub, pub_len,
                              fake_rng, &alice_rng) != 0 ||
      mbedtls_dhm_read_public(&bob, alice_pub, pub_len) != 0 ||
      mbedtls_dhm_make_public(&bob, (int) pub_len, bob_pub, pub_len,
                              fake_rng, &bob_rng) != 0 ||
      mbedtls_dhm_read_public(&alice, bob_pub, pub_len) != 0) {
    goto cleanup;
  }

  if (mbedtls_dhm_calc_secret(&alice, alice_secret, sizeof(alice_secret),
                              &alice_secret_len, fake_rng, &alice_rng) != 0 ||
      mbedtls_dhm_calc_secret(&bob, bob_secret, sizeof(bob_secret),
                              &bob_secret_len, fake_rng, &bob_rng) != 0) {
    goto cleanup;
  }
