...
static int run_api(int randomize, int iter_index) {
  ...
  if (mbedtls_ecdsa_sign(&ctx.MBEDTLS_PRIVATE(grp), &r, &s, &ctx.MBEDTLS_PRIVATE(d),
                         hash, sizeof(hash), fake_rng, &rng) != 0) {
    goto cleanup;
  }

  if (mbedtls_ecdsa_verify(&ctx.MBEDTLS_PRIVATE(grp), hash, sizeof(hash), &ctx.MBEDTLS_PRIVATE(Q), &r, &s) != 0) {
    goto cleanup;
  }
